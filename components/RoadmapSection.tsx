        ...
        <div ref={scrollRef} className="flex gap-4 sm:gap-5 overflow-x-auto scrollbar-hide pb-4 -mx-4 px-4 sm:-mx-6 sm:px-6">
          {events.map((m, i) => {
            ...
            return (
              <div
                key={m.id}
                className={`flex-shrink-0 w-[75vw] sm:w-[280px] md:w-[300px] rounded-2xl p-6 sm:p-7 flex flex-col relative overflow-hidden transition-all duration-200 hover:-translate-y-1 ${
                  m.is_done
              ...
              >
                ...
                <span className={`absolute top-4 right-5 font-accent text-7xl font-extrabold leading-none select-none ${
                  m.is_done ? "text-campaign-lime/15" : "text-primary-foreground/5"
                }`}>
                ...
                <div className={`w-11 h-11 rounded-xl flex items-center justify-center mb-6 ${
                  m.is_done ? "gradient-lime" : "bg-primary-foreground/10"
                }`}>
                  <Icon className={`w-5 h-5 ${m.is_done ? "text-accent-foreground" : "text-primary-foreground/40"}`} />
                </div>
                ...
                <h3 className={`font-accent font-extrabold text-lg mb-3 uppercase tracking-wide ${
                  m.is_done ? "text-primary-foreground" : "text-primary-foreground/35"
                }`}>
                  {m.title}
                </h3>
                ...
                <p className={`text-sm leading-relaxed mt-auto ${
                  m.is_done ? "text-primary-foreground/60" : "text-primary-foreground/25"
                }`}>
                  {m.description}
                </p>
              </div>
