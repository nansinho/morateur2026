...
const MilestoneCard = ({ m, i, isLeft }: { m: typeof milestones[0]; i: number; isLeft: boolean }) => {
  ...
  return (
    ...
    >
      ...
      <div className="absolute left-6 md:left-1/2 -translate-x-1/2 z-10">
        <motion.div
          className={`w-14 h-14 rounded-full flex items-center justify-center border-2 transition-all duration-700 ${
            m.done ? "gradient-green border-transparent glow-green" : "bg-primary border-primary-foreground/10"
          }`}
          animate={inView && !m.done ? {
            borderColor: ["hsl(0 0% 100% / 0.1)", "hsl(160 84% 39% / 0.3)", "hsl(0 0% 100% / 0.1)"],
            scale: [1, 1.08, 1],
          } : inView && m.done ? { scale: [0.8, 1.15, 1] } : {}}
          transition={{ duration: 2.5, repeat: m.done ? 0 : Infinity }}
        >
          <m.icon className={`w-6 h-6 ${m.done ? "text-primary-foreground" : "text-primary-foreground/30"}`} />
        </motion.div>
      </div>
      ...
      } ${m.done ? "bg-gradient-to-r from-campaign-green/40 to-campaign-green/10" : "bg-primary-foreground/[0.06]"}`} />

      {/* Card */}
      <div className={`ml-20 md:ml-0 md:w-[calc(50%-4rem)] ${isLeft ? "md:mr-auto md:pr-4" : "md:ml-auto md:pl-4"}`}>
        <motion.div
          className={`rounded-2xl p-7 border overflow-hidden relative transition-all duration-700 ${
            m.done
              ? "glass-card !border-campaign-green/20"
              : "border-primary-foreground/[0.06]"
          }`}
          style={{
            background: m.done
              ? undefined
              : `linear-gradient(135deg, hsl(0 0% 100% / ${inView ? 0.04 : 0.01}), hsl(0 0% 100% / ${inView ? 0.02 : 0.005}))`,
          }}
        ...
        >
          {m.done && (
            <>
              ...
              <div className="absolute inset-0 bg-campaign-green/[0.03]" />
            </>
          )}

          <div className="relative z-10">
            <div className="flex items-center justify-between mb-2">
              <span className={`text-xs font-bold uppercase tracking-wider ${m.done ? "text-campaign-green" : "text-primary-foreground/25"}`}>
                {m.date}
              </span>
              {m.done && (
                <motion.div
            ...
            <h3 className={`font-heading font-bold text-lg mt-1 ${m.done ? "text-primary-foreground" : "text-primary-foreground/50"}`}>
              {m.title}
            </h3>
            <p className={`text-sm mt-2 leading-relaxed ${m.done ? "text-primary-foreground/40" : "text-primary-foreground/20"}`}>
              {m.desc}
            </p>
          </div>
        </motion.div>
    ...
    <section ref={ref} id="roadmap" className="py-32 bg-primary relative overflow-hidden">
      ...
      <div className="container mx-auto px-6 relative z-10">
        ...
        >
          ...
          <h2 className="font-heading text-5xl md:text-7xl font-extrabold text-primary-foreground leading-tight mt-2">
        ...
        >
          <span className="text-campaign-green font-heading text-4xl font-extrabold drop-shadow-[0_0_20px_hsl(160,84%,39%,0.3)]">
            {progressPercent}%
          </span>
          <p className="text-primary-foreground/25 text-xs mt-2 uppercase tracking-[0.2em]">accompli</p>
        </motion.div>

        <div className="relative max-w-3xl mx-auto">
          {/* Track */}
          <div className="absolute left-6 md:left-1/2 md:-translate-x-px top-0 bottom-0 w-[3px] bg-primary-foreground/[0.04] rounded-full" />
          <motion.div
            ...
            style={{
              ...
              boxShadow: "0 0 25px hsl(160 84% 39% / 0.5), 0 0 50px hsl(160 84% 39% / 0.2)",
            }}
          />
